...
local gnome_sort = require 'gnome_sort'
...
run('Empty arrays', function()
  local t = {}
  assert(same(sort(clone(t)),gnome_sort(t)))
end)
...
run('Array sorted in reverse', function()
  ...
  assert(same(sort(clone(t), comp),gnome_sort(t, comp)))
end)

run('Array containing multiple occurences of the same value', function()
  local t = {4, 4, 8, 2, 2}
  local comp = function(a,b) return a <= b end
  assert(same(sort(clone(t)),gnome_sort(t, comp)))
  local t = {0, 0, 0, 0, 0}
  local comp = function(a,b) return a <= b end
  assert(same(sort(clone(t)),gnome_sort(t, comp)))
end)
