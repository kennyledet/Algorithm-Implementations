-- Tests for merge_sort.lua
local mergesort = require 'merge_sort'
...
-- Generates a table of n randm values
...
run('Empty arrays', function()
	local t = {}
	assert(is_sorted(mergesort({})))
end)

run('Already sorted array', function()
	local t = {1, 2, 3, 4, 5}
	assert(is_sorted(mergesort(t)))
end)

run('Sorting a large array (1e4 values)', function()
	local t = gen(1e4)
	assert(is_sorted(mergesort(t)))
	assert(is_sorted(mergesort(t, ge), ge))
end)
